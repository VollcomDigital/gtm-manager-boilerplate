...
test("loadRepoConfig: merges containers by key and workspace entities by name", async () => {
  const tmpDir = await fs.mkdtemp(path.join(process.cwd(), ".tmp-gtm-iac-repo-config-"));
  try {
...
test("loadRepoConfig: rejects config paths outside workspace root", async () => {
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "gtm-iac-outside-repo-config-"));
  const outsidePath = path.join(tmpDir, "outside.yml");
  try {
    await fs.writeFile(
      outsidePath,
      ["schemaVersion: 1", "containers:", "  - key: sample", "    target:", "      accountId: \"1\"", "      containerId: \"2\""].join("\n"),
      "utf-8"
    );
    await assert.rejects(
      loadRepoConfig(outsidePath),
